...
namespace QQBotCodePlugin.QQBot.abilities
{
    public class voice : IEventHandler
    {
        ...
        public void Register(Bot bot)
        {
            ...
            httpService = new HttpService(httpClient, bot.getConsole(), bot);
            List<string> description = bot.helpCommandHelper.getCommands("普通功能");
            description.Add("/说 [角色] [文本] - 进行语音合成 寄了~!~!~!~!");
        ...
        private async void OnMessageReceived(object sender, MessageEvent e)
        {
            ...
            if (match.Success)
            {
                ...
                {
                    await bot.Message.sendMessage(e.GroupId, e.MessageId, "角色未找到\n输入 /说 查看角色列表");
                    return;
                }
                string mp3 = await getMP3Url(role, line);
                bot.getLogger().Debug(mp3);
                string request = await bot.Message.sendVoiceMessage(e.GroupId, mp3);
                if (request.Contains("语音转换失败, 请检查语音文件是否正常"))
                {
                    await bot.Message.sendMessage(e.GroupId, e.MessageId, "语音API出现错误请稍后再试");
                }
            }
            else
            {
                ...
                {
                    await bot.Message.sendMessage(e.GroupId, e.MessageId, "无法获取 characters 数组");
                    return;
                }
                await bot.Message.sendMessage(e.GroupId, e.MessageId, charactersString + "\n\n用法: /说 角色 台词");
            }
        }
