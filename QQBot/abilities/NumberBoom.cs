...
namespace QQBotCodePlugin.QQBot.abilities
{
    public class NumberBoom : IEventHandler
    {
        ...
        private async void Bot_GroupReceived(object sender, utils.GroupMessageReceivedEvent e)
        {
            ...
            long groupId = e.Message.GroupId;
            ...
            switch (parts.Length)
            {
                case 1 when IsGameRunning(groupId):
                    await bot.Message.sendMessage(groupId, e.Message.MessageId, ProcessGuess(e.Message.Sender.UserId, groupId, e.Message.Sender.Nickname, message));
                    break;
                case 2:
                    HandleCommand(groupId, e.Message.MessageId, e.Message.Sender.UserId, parts[1], e.Message.Sender.Nickname);
                    break;
                default:
                    await bot.Message.sendMessage(groupId, e.Message.MessageId, "用法:\n/数字炸弹 join\n/数字炸弹 leave\n/数字炸弹 list\n数字炸弹 start\n/数字炸弹 stop\n详细信息请输入/help");
                    break;
            }
        }
